<script>
    ...
    $: {
        ...
        setTimeout(() => {
            ...
            document.getElementById("replay-scorer").style.fontSize = "38px";
            resize_to_fit(document.getElementById("replay-scorer"), 260);
            console.log("testscorer")
        }, 500)
        }
        if($goalData?.assister && isMounted){
            console.log("testassister")
            document.getElementById("replay-assister").style.fontSize = "35px";
            resize_to_fit(document.getElementById("replay-assister"), 260);
        }
...
<main>
    ...
    >
    <svg xmlns="http://www.w3.org/2000/svg" transform="translate(-9 , 845)" xmlns:xlink="http://www.w3.org/1999/xlink" width="586" height="111" viewBox="0 0 585.85 111.48">
        <defs>
          ...
          <g id="replay-card">
            ...
            <svg
                    ...
                    >
                        <g
                            id="goalspeed-svg"
                            style="fill: black; transform: scale(.4);"
                        >
                            <path
            ...
            <text id="replay-scorer" dominant-baseline="middle" transform="translate(70.13 {$goalData?.assister == "" ? 53.15 : 37.15})" style="font-size: 45px;fill: #fff;font-family: IntegralCF-Regular, Integral CF">{$goalData?.scorer ?? ""}</text>
            <text id="replay-assister" dominant-baseline="middle" transform="translate(70.13 82.15)" style="font-size: 35px;fill: #fff;font-family: IntegralCF-Regular, Integral CF; opacity: {$goalData?.assister == "" ? 0 : 1}">{$goalData?.assister ?? ""}</text>
            <text id="replay-goalspeed" transform="translate(430 62)" style="font-size: 35px; fill: #fff;font-family:IntegralCF-Regular, Integral CF">{$goalData?.speed ?? ""}</text>
        </g>
        </g>
      </svg>
